...
Credentials can change while the machine stays the same — a VS Code tunnel access token rotates, or you generate a fresh access link. Click **Re-pair** on the server row to update it in place: paste a new access link, or enable **Connect through a VS Code tunnel** and enter the current tunnel URL and access token. The saved server keeps its name, selection as Active Server, and paired identity; Orca verifies the new credentials and reconnects immediately.
